...
(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        define(['tracking', 'pca', 'jquery'], function (tracking, pca, jquery) {
        ...
        module.exports = factory(require('tracking'), require('pca'), jQuery);
    } else {
        root.facerecognition = factory(root.tracking, root.pca, root.jQuery);
    }
}(this, function (tracking, pca, $) {
    ...
    FaceRecognition.prototype = {
        ...
        isMoved: function (rect1, rect2, threshold) {
            var deltaX = 0, deltaY = 0, deltaWidth = 0, deltaHeight = 0;
            deltaX = Math.abs(rect1.x - rect2.x);
            deltaY = Math.abs(rect1.y - rect2.y);
            deltaWidth = Math.abs(rect1.width - rect2.width);
            deltaHeight = Math.abs(rect1.height - rect2.height);
            console.log('deltaX :' + deltaX);
            console.log('deltaY :' + deltaY);
            return deltaX > threshold | deltaY > threshold | deltaHeight > threshold | deltaWidth > threshold;
        },
        ...
        captureVideo: function (rect) {
            ...
            this.captureCallback(pca.Image.grayscale(pca.Image.normalize(canvas2, this.dimen.height, this.dimen.width)).toDataURL(),
                pca.Image.grayscale(pca.Image.normalize(canvas2, this.dimen.height, this.dimen.width)).getContext('2d').getImageData(0, 0, rect.width, rect.height).data);
        },
